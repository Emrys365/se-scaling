...
class ESPnetEnhancementModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.keep_1ch_prob = keep_1ch_prob
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert speech_ref.size(2) == speech_mix.size(1), (speech_ref.shape, speech_mix.shape)
        ...
        if "dereverb_ref1" in kwargs:
            ...
            assert dereverb_speech_ref.size(2) == speech_mix.size(1), (dereverb_speech_ref.shape, speech_mix.shape)
        else:
        ...
        if speech_mix.ndim > 2:
            num = np.random.rand()
            if num > self.keep_1ch_prob:
                # convert multi-channel signals into single-channel
                speech_mix = speech_mix[..., 0]
                if speech_ref[0].ndim > 2:
                    speech_ref = [sr[..., 0] for sr in speech_ref]
                if noise_ref is not None and noise_ref[0].ndim > 2:
                    noise_ref = [nr[..., 0] for nr in noise_ref]
                if dereverb_speech_ref is not None and dereverb_speech_ref[0].ndim > 2:
                    dereverb_speech_ref = [dr[..., 0] for dr in dereverb_speech_ref]
            # elif num > 1.0 / 3:
            else:
                # convert multi-channel signals into 2-channel
                chs = 4
                speech_mix = speech_mix[..., :chs]
                if speech_ref[0].ndim > 2:
                    speech_ref = [sr[..., :chs] for sr in speech_ref]
                if noise_ref is not None and noise_ref[0].ndim > 2:
                    noise_ref = [nr[..., :chs] for nr in noise_ref]
                if dereverb_speech_ref is not None and dereverb_speech_ref[0].ndim > 2:
                    dereverb_speech_ref = [dr[..., :chs] for dr in dereverb_speech_ref]
        ...
        if "utt2fs" in kwargs:
            ...
            if self.always_forward_in_48k:
                tgt_fs = 48000
            else:
                if speech_mix.ndim == 2:
                    #fs_opts = [8000]
                    fs_opts = [8000, 16000, 24000, 32000, 44100, 48000]
                    fs_opts = [x for x in fs_opts if x <= fs]
                    tgt_fs = np.random.choice(fs_opts)
                else:
                    tgt_fs = 8000
                    #tgt_fs = fs
            ...
            if fs != tgt_fs:
                ...
                if speech_mix.ndim > 2:
                    speech_mix = speech_mix.transpose(1, 2).contiguous()
                if not self.always_forward_in_48k:
                    if speech_ref[0].ndim > 2:
                        speech_ref = [
                            torchaudio.functional.resample(
                                sr.transpose(1, 2).contiguous(), fs, tgt_fs
                            )
                            .transpose(1, 2)
                            .contiguous()
                        ...
                        speech_ref = [
                            torchaudio.functional.resample(sr, fs, tgt_fs)
                            for sr in speech_ref
                        ]
                    if dereverb_speech_ref is not None:
                        if dereverb_speech_ref[0].ndim > 2:
                            dereverb_speech_ref = [
                                torchaudio.functional.resample(
                                    dr.transpose(1, 2).contiguous(), fs, tgt_fs
                                )
                                .transpose(1, 2)
                                .contiguous()
                            ...
                            dereverb_speech_ref = [
                                torchaudio.functional.resample(dr, fs, tgt_fs)
                                for dr in dereverb_speech_ref
                            ]
            fs = tgt_fs

            # Adaptively adjust the STFT/iSTFT window/hop sizes for
            # BSRNNSeparator and TFGridNetV3
            if not isinstance(self.separator, (BSRNNSeparator, TFGridNetV3)):
                fs = None
        ...
        if "num_spk" in kwargs:
            assert int(kwargs["num_spk"][0].cpu().numpy()) == num_spk, kwargs["num_spk"]
        ...
        try:
            ...
            print(f"speech_mix: {speech_mix.shape}, fs: {fs}", flush=True)
            raise
        ...
        loss = loss + others.get("loss", 0.0)
        if category is not None and additional["mode"] == "both":
            assert dereverb_speech_ref is not None
            dereverb_pre = [others[f"dereverb{spk + 1}"] for spk in range(self.num_spk)]
            loss2, stats2, _, _ = self.forward_loss(
                dereverb_pre,
                speech_lengths,
                feature_mix,
                None,
            ...
            for k in stats.keys():
                if stats[k].ndim != 0:
                    continue
                if torch.isfinite(stats[k]):
                    if torch.isfinite(stats2[k]):
                        stats[k] = (stats[k] + stats2[k]) / 2
                else:
                    stats[k] = stats2[k]
    ...
    def forward_loss(
        ...
        if "existance_probability" in others:
            with torch.cuda.amp.autocast(enabled=False):
                ...
                exist, non_exist = (
                    others["existance_probability"][..., :num_spk].float(),
                    others["existance_probability"][..., num_spk].float(),
                )
                ...
                stats["attractor_exist_prob"] = (
                    others["existance_probability"][..., :num_spk].mean().detach()
                )
